#!/usr/bin/env bash
#
# TD3 env sweep: td3 across envs {HalfCheetah-v5, Ant-v5}, seeds
# {331, 958, 14577, 43611, 85062} -- 10 runs total.
#
# Env-specific hyperparameters: TD3Config's docstring (configs/config.py)
# notes the paper applies its hyperparameters unmodified across every
# MuJoCo task, so no --algo-config-overrides file is needed for either env.
# The one setting that does vary by env is --warmup-steps: the paper uses
# 10,000 steps for HalfCheetah-v1/Ant-v1 ("stable length environments")
# rather than the CLI's default of 5,000, so both envs here get
# --warmup-steps 10000 to match.
#
...
#
#   screen -S td3_env_sweep
#   ./run_td3_env_sweep.sh
#   # Ctrl-A D to detach; reattach later with: screen -r td3_env_sweep
#
# Check progress at any time (from another terminal / after reattaching)
# with:
#   ./check_td3_env_sweep.sh
#
set -uo pipefail
cd "$(dirname "$0")"
...
# use TD3Config defaults".
declare -A ENV_OVERRIDES=(
    ["HalfCheetah-v5"]=""
    ["Ant-v5"]=""
)

# Per-env --warmup-steps; both envs here are the paper's "stable length"
# tasks, which use 10,000 rather than the CLI default of 5,000.
declare -A ENV_WARMUP_STEPS=(
    ["HalfCheetah-v5"]=10000
    ["Ant-v5"]=10000
)

STATUS_FILE="results/td3/_env_sweep_status.json"
LOG_FILE="results/td3/_env_sweep.log"
...
for env in "${ENVS[@]}"; do
    ...
    warmup_steps="${ENV_WARMUP_STEPS[$env]}"
    for seed in "${SEEDS[@]}"; do
        idx=$((idx + 1))

        log "[$idx/$TOTAL] Starting: algo=$ALGO env=$env seed=$seed overrides=${overrides:-none} warmup_steps=$warmup_steps"
        update_run_status "$idx" \
            "status=\"running\"" \
            "started_utc=\"$(date -u +%Y-%m-%dT%H:%M:%S)\""

        run_output="$(mktemp)"
        cmd=(sudo "$PYTHON_BIN" run_experiment.py --algo "$ALGO" --env "$env" --seed "$seed" --warmup-steps "$warmup_steps")
        if [[ -n "$overrides" ]]; then
