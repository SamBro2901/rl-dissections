#!/usr/bin/env bash
#
# MBPO env sweep: mbpo across envs {HalfCheetah-v5, Ant-v5}, seeds
# {331, 958, 14577, 43611, 85062} -- 10 runs total.
#
# Env-specific hyperparameters: HalfCheetah-v5 uses MBPOConfig's plain
# defaults (configs/config.py notes these already reproduce the paper's
# HalfCheetah-v2 setting), Ant-v5 uses configs/overrides/mbpo_ant.json
# (the paper's scheduled model-rollout-length setting for Ant, per the
# MBPOConfig docstring). No override file is passed for HalfCheetah.
#
# Note: MBPO run times are noticeably less predictable than SAC's -- the
# dynamics-model retrain step (model_train_freq / model_max_train_epochs /
# model_train_patience) early-stops on a holdout metric that varies run to
# run, so don't be surprised if two runs of the same config differ a lot in
# wall-clock time.
#
...
#
#   screen -S mbpo_env_sweep
#   ./run_mbpo_env_sweep.sh
#   # Ctrl-A D to detach; reattach later with: screen -r mbpo_env_sweep
#
# Check progress at any time (from another terminal / after reattaching)
# with:
#   ./check_mbpo_env_sweep.sh
#
set -uo pipefail
cd "$(dirname "$0")"
...
# use MBPOConfig defaults".
declare -A ENV_OVERRIDES=(
    ["HalfCheetah-v5"]=""
    ["Ant-v5"]="configs/overrides/mbpo_ant.json"
)

STATUS_FILE="results/mbpo/_env_sweep_status.json"
LOG_FILE="results/mbpo/_env_sweep.log"
