#!/usr/bin/env bash
#
# Progress checker for run_mbpo_env_sweep.sh. Safe to run any time, from any
...
if [[ ! -f "$STATUS_FILE" ]]; then
    echo "No sweep status file found at $STATUS_FILE -- has run_mbpo_env_sweep.sh been started?"
    exit 1
fi
...
echo "MBPO env sweep progress (started $started UTC, last update $updated UTC)"
echo "Overall: $(bar "$done_count" "$total")  ($failed_count failed)"
echo

if [[ "$failed_count" -gt 0 ]]; then
    echo "Failed runs:"
    jq -r --argjson total "$total" '.runs[] | select(.status == "failed") | "  [\(.index)/\($total)] env=\(.env) seed=\(.seed) exit_code=\(.exit_code)"' "$STATUS_FILE"
    echo
fi
...
cur_env="$(echo "$current" | jq -r '.env')"
cur_seed="$(echo "$current" | jq -r '.seed')"
cur_started="$(echo "$current" | jq -r '.started_utc')"

echo "Current run: [$cur_idx/$total] env=$cur_env seed=$cur_seed (started $cur_started UTC)"

run_dir="$(ls -td "results/mbpo/${cur_env}/seed_${cur_seed}"/*/ 2>/dev/null | head -1)"
if [[ -n "$run_dir" && -f "${run_dir}run.log" ]]; then
    ...
    if [[ -n "$epoch_line" ]]; then
        ...
        phase_line="$(grep -E 'Settling|Recording idle|Starting warmup|Starting measured training|Waiting for thermal|retrain' "${run_dir}run.log" | tail -1)"
