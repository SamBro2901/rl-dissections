#!/usr/bin/env bash
#
# Batch-size sweep: batch_size in {256, 512, 1024}, HalfCheetah-v5 only,
# canonical 5 seeds {331, 958, 14577, 43611, 85062}, for:
#   - sac   (batch sizes 512, 1024 -- 256 is SACConfig's default, already
#            covered by the existing canonical HalfCheetah-v5 runs)
...
#
#   screen -S batch_size_sweep
#   ./run_batch_size_sweep.sh
#   # Ctrl-A D to detach; reattach later with: screen -r batch_size_sweep
#
...
# TD3's paper hyperparameters use 10,000 steps for HalfCheetah-v1 ("stable
# length environments"); SAC and MBPO use the CLI default.
...
override_file() {
    ...
    echo "configs/overrides/${algo}_batch${batch}.json"
}

STATUS_FILE="results/_batch_size_sweep_status.json"
LOG_FILE="results/_batch_size_sweep.log"
...
sudo shutdown -h +1 "rl-dissection batch-size sweep finished ($((TOTAL - fail_count))/$TOTAL runs OK) -- powering down"
