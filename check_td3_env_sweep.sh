#!/usr/bin/env bash
#
# Progress checker for run_td3_env_sweep.sh. Safe to run any time, from any
...
if [[ ! -f "$STATUS_FILE" ]]; then
    echo "No sweep status file found at $STATUS_FILE -- has run_td3_env_sweep.sh been started?"
    exit 1
fi
...
echo "TD3 env sweep progress (started $started UTC, last update $updated UTC)"
...
run_dir="$(ls -td "results/td3/${cur_env}/seed_${cur_seed}"/*/ 2>/dev/null | head -1)"
